...
<div class="mx-auto max-w-3xl rounded-xl bg-white p-6 shadow-sm">
    <h1 class="mb-4 text-2xl font-semibold">Добавить работника</h1>
    <form method="POST" action="{{ route('workers.store') }}" class="space-y-4">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <input name="email" type="email" value="{{ old('email') }}" required class="w-full rounded-md border border-slate-300 px-3 py-2" />
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                ...
                <input name="password" type="password" required class="w-full rounded-md border border-slate-300 px-3 py-2" />
                @error('password')<p class="mt-1 text-sm text-rose-600">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Подтверждение пароля</label>
                <input name="password_confirmation" type="password" required class="w-full rounded-md border border-slate-300 px-3 py-2" />
            </div>
        </div>
        <div class="grid gap-4 md:grid-cols-2">
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700">Серия паспорта</label>
                <input name="passport_series" value="{{ old('passport_series') }}" class="w-full rounded-md border border-slate-300 px-3 py-2" />
            ...
            <div>
                ...
                <input name="passport_number" value="{{ old('passport_number') }}" class="w-full rounded-md border border-slate-300 px-3 py-2" />
        ...
        <div>
            ...
            <input name="salary" type="number" step="0.01" value="{{ old('salary', 0) }}" required class="w-full rounded-md border border-slate-300 px-3 py-2" />
            ...
            <div class="grid gap-3 md:grid-cols-2">
                @foreach($cages as $cage)
                    <label class="flex items-center gap-3 rounded-md border border-slate-200 bg-slate-50 p-3">
                        <input type="checkbox" name="cage_ids[]" value="{{ $cage->id }}" class="h-4 w-4 rounded" />
