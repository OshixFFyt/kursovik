...
<div class="container mx-auto mt-4">
    <div class="card shadow-sm border-0">
        <div class="card-body">
            <h1 class="mb-4 card-title"><i class="bi bi-plus-circle"></i> Добавить работника</h1>
            <form method="POST" action="{{ route('workers.store') }}">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input name="passport_series" value="{{ old('passport_series') }}" inputmode="numeric" pattern="[0-9]+" title="Оно должно содержать только цифры" class="form-control" />
                        @error('passport_series')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Номер паспорта</label>
                        <input name="passport_number" value="{{ old('passport_number') }}" inputmode="numeric" pattern="[0-9]+" title="Оно должно содержать только цифры" class="form-control" />
                        @error('passport_number')<div class="text-danger small mt-1">{{ $message }}</div>@enderror
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label">Зарплата</label>
                    <input name="salary" type="number" step="0.01" min="0" value="{{ old('salary', 0) }}" required class="form-control" />
                        ...
                        @foreach($cages as $cage)
                            <div class="col-md-6 mb-2">
                                <div class="form-check">
                                    <input type="checkbox" name="cage_ids[]" value="{{ $cage->id }}" class="form-check-input" id="cage_{{ $cage->id }}" />
